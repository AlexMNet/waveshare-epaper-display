...
def main():
    ...
    def get_messages():
        ...
        try:
            response = requests.get(url, headers={"Authorization": f"Bearer {notion_apikey}", "Notion-Version": "2022-06-28"})
            data = response.json()
            message = data['results'][0]['paragraph']['rich_text'][0]['plain_text']
            print(message)
            return message
